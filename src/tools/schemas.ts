...
export const VoucherSchema = BaseVoucherSchema.extend({
  CantReg: z.number().min(1).describe("Cantidad de registros del detalle del comprobante o lote de comprobantes de ingreso"),
  CbteDesde: z.number().min(1).describe("Nro. de comprobante desde. Rango 1 – 99999999"),
  CbteHasta: z.number().min(1).describe("Nro. de comprobante hasta. Rango 1 – 99999999"),
  fullResponse: z.boolean().optional().describe("Si es true, devuelve la respuesta completa del WS. Si es false o no se especifica, devuelve solo CAE y CAEFchVto"),
});
...
export const GetInvoicesInDateRangeSchema = z.object({
  PtoVta: z.number().min(1).describe("Punto de venta del comprobante"),
  CbteTipo: z.number().min(1).describe("Tipo de comprobante"),
  fechaDesde: z.string().regex(/^\d{8}$/).describe("Fecha desde en formato YYYYMMDD"),
  fechaHasta: z.string().regex(/^\d{8}$/).describe("Fecha hasta en formato YYYYMMDD"),
  batchSize: z.number().min(1).max(50).default(20).describe("Tamaño del lote para consultas paralelas (1-50, default: 20)"),
  includeDetails: z.boolean().default(false).describe("Incluir detalles completos de cada comprobante (default: false)"),
  maxVouchers: z.number().min(1).max(1000).default(500).describe("Límite máximo de comprobantes a procesar (1-1000, default: 500)")
});
